...
//        $user['formated_frozen_money'] = price_format(0);
    ...
    $smarty->assign('special_ranks', get_rank_list(true));
    $smarty->assign('profile', getuserProfile($user['user_id']));
    $smarty->display('user_info.htm');
}
...
function user_list()
{
    ...
    if ($result === false) {
        ...
        $sql = "SELECT user_id, user_name, mobile_phone, email, is_validated, user_money, frozen_money, credit_line, rank_points, pay_points, reg_time " .
...
//    var_dump($result);
    $profile = $result[0];
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('users_friend') . " where user_id=" . $userId;
    $profile['friends'] = $GLOBALS['db']->getAll($sql);
    return $profile;
}
